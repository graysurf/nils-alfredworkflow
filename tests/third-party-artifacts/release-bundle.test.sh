...
test_root="$(mktemp -d "${TMPDIR:-/tmp}/third-party-artifacts-release-bundle.test.XXXXXX")"
...
__NOTICE__
  ...
  cat >"$dist_dir/workflows-v0.0.0-test.zip" <<'__ZIP__'
fixture-zip-payload
__ZIP__

  cat >"$dist_dir/workflow-clear-quarantine-standalone.sh" <<'__SCRIPT__'
#!/usr/bin/env bash
echo fixture
__SCRIPT__
  ...
  cp "$fixture/THIRD_PARTY_NOTICES.md" "$dist_dir/THIRD_PARTY_NOTICES.md"
  ...
  write_sha256 "$dist_dir/THIRD_PARTY_NOTICES.md" "$dist_dir/THIRD_PARTY_NOTICES.md.sha256"
...
test_release_bundle_fails_when_notices_artifact_is_missing() {
  local fixture
  fixture="$(setup_fixture)"
  rm -f "$fixture/dist/release-bundles/THIRD_PARTY_NOTICES.md"

  run_audit "$fixture" --tag v0.0.0-test --dist-dir dist/release-bundles
  if ! assert_eq "1" "$last_rc" "release bundle missing notices exit code"; then
    dump_last_run
    return 1
  fi
  ...
  local output
  output="$(read_combined_output "$fixture")"
  if ! assert_contains "$output" "FAIL [release-bundle-third-party] third-party notices artifact missing:" "release bundle missing notices FAIL detail"; then
    dump_last_run
    return 1
  fi
  if ! assert_contains "$output" "Result: FAIL (release bundle third-party compliance issues detected)" "release bundle missing notices FAIL result"; then
    dump_last_run
    return 1
  fi
...
run_test test_release_bundle_fails_when_notices_artifact_is_missing

...
test_root="$(mktemp -d "${TMPDIR:-/tmp}/third-party-artifacts-audit.test.XXXXXX")"
...
setup_fixture() {
  ...
  mkdir -p "$fixture/scripts/ci" "$fixture/scripts"

  cp "$repo_root/scripts/ci/third-party-artifacts-audit.sh" "$fixture/scripts/ci/third-party-artifacts-audit.sh"
  chmod +x "$fixture/scripts/ci/third-party-artifacts-audit.sh"

  cat >"$fixture/scripts/generate-third-party-artifacts.sh" <<'__GEN__'
#!/usr/bin/env bash
set -euo pipefail
...
clean)
  ...
  ;;
missing)
  echo "FAIL [check] missing output artifact: $PWD/THIRD_PARTY_LICENSES.md (run --write first)" >&2
  exit 1
  ;;
drift)
...
# fixture license artifact
__LICENSE__

  cat >"$fixture/THIRD_PARTY_NOTICES.md" <<'__NOTICE__'
# fixture notices artifact
__NOTICE__
...
run_audit() {
  ...
  (
    cd "$fixture"
    AUDIT_FIXTURE_STATE="$state" bash "$fixture/scripts/ci/third-party-artifacts-audit.sh" "$@"
...
test_clean_passes() {
  ...
  if ! assert_contains "$output" "PASS [third-party-artifacts]" "clean PASS output"; then
    dump_last_run
    return 1
  fi
...
test_missing_non_strict_warns() {
  ...
  local output
  output="$(read_combined_output "$fixture")"
  if ! assert_contains "$output" "WARN [third-party-artifacts]" "missing WARN output"; then
    dump_last_run
    return 1
  fi
...
test_missing_strict_fails() {
  ...
  local output
  output="$(read_combined_output "$fixture")"
  if ! assert_contains "$output" "WARN [third-party-artifacts]" "missing strict WARN output"; then
    dump_last_run
    return 1
  fi
  if ! assert_contains "$output" "FAIL [third-party-artifacts] strict mode treats warnings as failures" "missing strict FAIL output"; then
    dump_last_run
    return 1
  fi
...
test_drift_non_strict_warns() {
  ...
  local output
  output="$(read_combined_output "$fixture")"
  if ! assert_contains "$output" "WARN [third-party-artifacts] artifact drift detected" "drift WARN output"; then
    dump_last_run
    return 1
  fi
...
test_drift_strict_fails() {
  ...
  local output
  output="$(read_combined_output "$fixture")"
  if ! assert_contains "$output" "WARN [third-party-artifacts] artifact drift detected" "drift strict WARN output"; then
    dump_last_run
    return 1
  fi
  if ! assert_contains "$output" "FAIL [third-party-artifacts] strict mode treats warnings as failures" "drift strict FAIL output"; then
    dump_last_run
    return 1
  fi

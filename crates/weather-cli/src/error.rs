...
mod tests {
    ...
    fn error_exit_code_mapping_is_stable() {
        assert_eq!(AppError::user("bad input").exit_code(), 2);
        assert_eq!(AppError::runtime("boom").exit_code(), 1);
    }

    #[test]
    fn error_runtime_with_trace_formats_message() {
        let err = AppError::runtime_with_trace(
            "failed",
            &["open_meteo: timeout".to_string(), "met_no: 503".to_string()],
        );

        assert!(err.message.contains("provider trace"));
        assert!(err.message.contains("open_meteo: timeout"));
    }
}

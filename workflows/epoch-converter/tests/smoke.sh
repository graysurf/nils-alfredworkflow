...
[[ "$(toml_string "$manifest" id)" == "epoch-converter" ]] || fail "workflow id mismatch"
[[ "$(toml_string "$manifest" rust_binary)" == "epoch-cli" ]] || fail "rust_binary must be epoch-cli"
...
if ! rg -n '^EPOCH_CLI_BIN[[:space:]]*=[[:space:]]*""' "$manifest" >/dev/null; then
  fail "EPOCH_CLI_BIN default must be empty"
fi
...
if [[ -f "$release_cli" ]]; then
  release_backup="$tmp_dir/epoch-cli.release.backup"
...
cat >"$tmp_dir/stubs/epoch-cli-ok" <<'EOS'
#!/usr/bin/env bash
set -euo pipefail
[[ "${1:-}" == "convert" ]] || exit 9
[[ "${2:-}" == "--query" ]] || exit 9
query="${3:-}"
printf '{"items":[{"title":"epoch->local","subtitle":"query=%s","arg":"2025-01-02 03:04:05","valid":true}]}' "$query"
printf '\n'
EOS
chmod +x "$tmp_dir/stubs/epoch-cli-ok"

cat >"$tmp_dir/stubs/epoch-cli-invalid" <<'EOS'
#!/usr/bin/env bash
set -euo pipefail
echo "invalid input: unsupported datetime format" >&2
exit 2
EOS
chmod +x "$tmp_dir/stubs/epoch-cli-invalid"

cat >"$tmp_dir/stubs/epoch-cli-runtime" <<'EOS'
#!/usr/bin/env bash
set -euo pipefail
echo "io error: failed to read timezone data" >&2
exit 3
EOS
chmod +x "$tmp_dir/stubs/epoch-cli-runtime"

cat >"$tmp_dir/stubs/epoch-cli-malformed" <<'EOS'
#!/usr/bin/env bash
set -euo pipefail
printf '{"unexpected":"shape"}\n'
EOS
chmod +x "$tmp_dir/stubs/epoch-cli-malformed"

success_json="$({ EPOCH_CLI_BIN="$tmp_dir/stubs/epoch-cli-ok" "$workflow_dir/scripts/script_filter.sh" "1700000000"; })"
assert_jq_json "$success_json" '.items | type == "array" and length == 1' "script_filter success must output items array"
assert_jq_json "$success_json" '.items[0].title == "epoch->local"' "script_filter should forward successful JSON"

empty_query_json="$({ EPOCH_CLI_BIN="$tmp_dir/stubs/epoch-cli-ok" "$workflow_dir/scripts/script_filter.sh" "   "; })"
assert_jq_json "$empty_query_json" '.items[0].title == "epoch->local"' "empty query should be forwarded to epoch-cli"
assert_jq_json "$empty_query_json" '.items[0].valid == true' "empty query from epoch-cli should remain valid"

invalid_json="$({ EPOCH_CLI_BIN="$tmp_dir/stubs/epoch-cli-invalid" "$workflow_dir/scripts/script_filter.sh" "not-a-date"; })"
assert_jq_json "$invalid_json" '.items | type == "array" and length == 1' "invalid input fallback must output single item"
assert_jq_json "$invalid_json" '.items[0].title == "Invalid input"' "invalid input title mapping mismatch"
assert_jq_json "$invalid_json" '.items[0].valid == false' "invalid input fallback item must be invalid"

runtime_json="$({ EPOCH_CLI_BIN="$tmp_dir/stubs/epoch-cli-runtime" "$workflow_dir/scripts/script_filter.sh" "1700000000"; })"
assert_jq_json "$runtime_json" '.items[0].title == "Epoch Converter runtime failure"' "runtime failure title mapping mismatch"

malformed_json="$({ EPOCH_CLI_BIN="$tmp_dir/stubs/epoch-cli-malformed" "$workflow_dir/scripts/script_filter.sh" "1700000000"; })"
assert_jq_json "$malformed_json" '.items[0].title == "Epoch Converter error"' "malformed JSON should fallback to generic error"
...
copied_missing_script="$missing_layout/workflows/epoch-converter/scripts/script_filter.sh"
...
missing_binary_json="$({ EPOCH_CLI_BIN="$missing_layout/does-not-exist/epoch-cli" "$copied_missing_script" "1700000000"; })"
assert_jq_json "$missing_binary_json" '.items[0].title == "epoch-cli binary not found"' "missing binary fallback title mismatch"
...
run_layout_check() {
  ...
  local copied_script="$layout/workflows/epoch-converter/scripts/script_filter.sh"
  ...
  case "$mode" in
  packaged)
    make_layout_cli "$layout/workflows/epoch-converter/bin/epoch-cli" "$marker"
    ;;
  release)
    make_layout_cli "$layout/target/release/epoch-cli" "$marker"
    ;;
  debug)
    make_layout_cli "$layout/target/debug/epoch-cli" "$marker"
    ;;
  *)
  ...
  assert_jq_json "$output" ".items[0].title == \"$marker\"" "script_filter failed to resolve $mode epoch-cli path"
}
...
#!/usr/bin/env bash
set -euo pipefail
if [[ "\$#" -eq 4 && "\$1" == "build" && "\$2" == "--release" && "\$3" == "-p" && "\$4" == "nils-epoch-cli" ]]; then
  mkdir -p "$repo_root/target/release"
  cat >"$repo_root/target/release/epoch-cli" <<'EOCLI'
#!/usr/bin/env bash
set -euo pipefail
printf '{"items":[]}\n'
EOCLI
  chmod +x "$repo_root/target/release/epoch-cli"
  exit 0
fi
...
PATH="$tmp_dir/bin:$PATH" "$repo_root/scripts/workflow-pack.sh" --id epoch-converter >/dev/null

packaged_dir="$repo_root/build/workflows/epoch-converter/pkg"
...
assert_file "$packaged_dir/bin/epoch-cli"
...
assert_jq_file "$packaged_json_file" '.objects[] | select(.uid=="70EEA820-E77B-42F3-A8D2-1A4D9E8E4A10") | .config.keyword == "epoch||ts"' "keyword trigger must support epoch and ts"
...
assert_jq_file "$packaged_json_file" '[.userconfigurationconfig[] | .variable] | sort == ["EPOCH_CLI_BIN"]' "user configuration variables mismatch"
assert_jq_file "$packaged_json_file" '.userconfigurationconfig[] | select(.variable=="EPOCH_CLI_BIN") | .config.required == false' "EPOCH_CLI_BIN must be optional"

echo "ok: epoch-converter smoke test"

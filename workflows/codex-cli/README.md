...
Run core `nils-codex-cli@0.4.7` operations from Alfred.
...
- `.alfredworkflow` bundles `codex-cli@0.4.7` (release-coupled runtime version).
...
```bash
cargo install nils-codex-cli --version 0.4.7
```
...
- Official package should bundle exactly `codex-cli@0.4.7`.
...
- Packaging binary resolution order:
  ...
  3. auto-install pinned `nils-codex-cli@0.4.7` from crates.io via `cargo install --locked --root <cache-root>`
- Useful overrides:

...
assert_jq_file "$packaged_json_file" '.objects | length == 17' "plist must contain eight script filters, eight hotkeys, and one action object"
...
assert_jq_file "$packaged_json_file" '[.objects[] | select(.type=="alfred.workflow.trigger.hotkey")] | length == 8' "plist must expose one dedicated hotkey trigger per script filter"

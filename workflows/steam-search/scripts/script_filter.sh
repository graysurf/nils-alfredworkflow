...
print_error_item() {
  ...
  elif [[ "$lower" == *"invalid steam_region"* || "$lower" == *"invalid steam_region_options"* || "$lower" == *"invalid steam_show_region_options"* || "$lower" == *"invalid steam_language"* || "$lower" == *"invalid steam_max_results"* || "$lower" == *"invalid steam_specials_max_results"* || "$lower" == *"invalid steam_search_api"* ]]; then
    title="Invalid Steam workflow config"
    subtitle="Check STEAM_REGION, STEAM_REGION_OPTIONS, STEAM_SHOW_REGION_OPTIONS, STEAM_LANGUAGE, STEAM_MAX_RESULTS, STEAM_SPECIALS_MAX_RESULTS, and STEAM_SEARCH_API."
...
# writable cover cache dir so steam-cli does not depend on Alfred's env casing.
resolve_cover_cache_dir() {
  local dir
  dir="${STEAM_COVER_CACHE_DIR:-${alfred_workflow_cache:-${ALFRED_WORKFLOW_CACHE:-}}}"
  if [[ -z "$dir" ]]; then
    dir="$(wfar_resolve_cache_dir "nils-steam-search-workflow")"
  fi
  printf '%s' "$dir"
}
...
steam_search_fetch_json() {
  ...
  cover_cache_dir="$(resolve_cover_cache_dir)"

  local json_output
  if json_output="$(STEAM_REGION="$STEAM_ACTIVE_REGION" STEAM_COVER_CACHE_DIR="$cover_cache_dir" "$steam_cli" search --query "$query" --output alfred-json 2>"$err_file")"; then
    rm -f "$err_file"

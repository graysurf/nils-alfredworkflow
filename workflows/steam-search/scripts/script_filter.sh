...
print_error_item() {
  ...
  elif [[ "$lower" == *"invalid steam_region"* || "$lower" == *"invalid steam_region_options"* || "$lower" == *"invalid steam_show_region_options"* || "$lower" == *"invalid steam_language"* || "$lower" == *"invalid steam_max_results"* || "$lower" == *"invalid steam_search_api"* ]]; then
    title="Invalid Steam workflow config"
    subtitle="Check STEAM_REGION, STEAM_REGION_OPTIONS, STEAM_SHOW_REGION_OPTIONS, STEAM_LANGUAGE, STEAM_MAX_RESULTS, and STEAM_SEARCH_API."

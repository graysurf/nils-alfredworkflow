...
| [Google Service](workflows/google-service/README.md) | `gs`, `gsa`, `gsd`, `gsm` | `gs` shows account status (optional all-account unread summary); `gsa` manages auth login/remove/switch; `gsd` supports Drive home/search/download; `gsm` supports Gmail unread/latest/search list. | Optional: `GOOGLE_CLI_CONFIG_DIR`, `GOOGLE_DRIVE_DOWNLOAD_DIR`, `GOOGLE_GS_SHOW_ALL_ACCOUNTS_UNREAD` |
...
| [Market Expression](workflows/market-expression/README.md) | `mx`, `market` | Show a prompt row plus favorite quotes on empty query, or evaluate market expressions (numeric: `+ - * /`, assets: `+ -`) with FX/crypto conversion and copy selected rows. | Optional: `MARKET_DEFAULT_FIAT`, `MARKET_FAVORITE_LIST` |
